...
function validateObject(schema, names) {
	...
	assertType(schema, 'atLeastOne', 'array', names);
	if (schema.atLeastOne !== undefined) {
		if (schema.atLeastOne.length < 2) {
			throw new Error('Schema: \'atLeastOne\' attribute: requires two or more items');
		}
		for (var i = 0, len = schema.atLeastOne.length; i < len; i++) {
			var property = schema.atLeastOne[i];
			switch(getType(property)) {
				case 'string':
					// nothing to validate
					break;
				default:
					throwInvalidType(names, '\'atLeastOne\' attribute: value \'' + property + '\'', property, 'either a string');
			}
		};
	}
}
